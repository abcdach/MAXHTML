...
function doConnect() {
    ...
    websocket.onclose = function(evt) {DOM_alert.hide();
        onClose(evt);
        Menu_Con_disconnect();
...
document.write('<script type="text/javascript" src="js/websock/Read_FILE.js"></script>');
...
function onMessage(evt) {
   ...
   // Term_Inp2.value(evt.data);
    ...
    if (typeof JSON_VALUE !== "undefined") {
        ...
        //#########################################################
        if (C_Val === 5) { // Device_OnlineStatus
            //--------------------------------------------------------------
				JSON_VALUE = JSON_PARSE.V;
				if (typeof JSON_VALUE === "undefined") return 1;
				V_Val = JSON_VALUE;
        	//--------------------------------------------------------------
				//TERMINAL("Device_OnlineStatus : " + D_Val + " VALUE : " + V_Val + '\n');
        	//--------------------------------------------------------------
				if(V_Val==="1"){
    ...
    if (typeof JSON_VALUE !== "undefined") {	
        ...
        case 0:
            //#--------------------------------------------------------------
				...
				if (typeof JSON_VALUE === "undefined") return 1;
				var Encrypted_Hex = JSON_VALUE;
            //#--------------------------------------------------------------
				//LOG_TERMINAL(FROM_SER + Encrypted_Hex + '\n');
			//#--------------------------------------------------------------
				var PassIN = dom__Connect_Inp0.value();
				dom__Connect_Inp0.value("");
				//LOG_TERMINAL(FROM_SER + PassIN + '\n');
				var PassIN_SHA1 = sha1(PassIN).substring(0,16).toUpperCase();
				var key = []; for( var i = 0; i < 16; i++ ) key.push(PassIN_SHA1.charCodeAt(i));
				//LOG_TERMINAL(FROM_SER +key + '\n');
				var str1 = '0123456789ABCDEF';
				var iv = [];  for( var i = 0; i < 16; i++ ) iv.push(str1.charCodeAt(i));
			//#--------------------------------------------------------------
			...
			//#--------------------------------------------------------------
				//LOG_TERMINAL(FROM_SER + decryptedText + '\n');		
			//#--------------------------------------------------------------
				Rev_decryptedText = decryptedText.split("").reverse().join("");
			//#--------------------------------------------------------------
				//LOG_TERMINAL(FROM_SER + Rev_decryptedText + '\n');		
			//#--------------------------------------------------------------
				var text = Rev_decryptedText;
				if(text.length != 16) text = "0123456789ABCDEF"
				var textBytes = aesjs.utils.utf8.toBytes(text);
				var aesCbc = new aesjs.ModeOfOperation.cbc(key, iv);
				var encryptedBytes = aesCbc.encrypt(textBytes);
				var encryptedHex = aesjs.utils.hex.fromBytes(encryptedBytes);
			//#--------------------------------------------------------------
				//LOG_TERMINAL(FROM_SER + encryptedHex + '\n');
			//#--------------------------------------------------------------
				var Msg = "{\"F\":\"0\",\"X\":\"" + encryptedHex + "\"}";
        ...
        case 5://Read Mediator code
            ...
            JSON_VALUE = JSON_PARSE.X;
        	if (typeof JSON_VALUE === "undefined") return 1;
            Read_ME_X = Number(JSON_VALUE);
            //#--------------------------------------------------------------
            JSON_VALUE = JSON_PARSE.Y;
        	if (typeof JSON_VALUE === "undefined") return 1;
            Read_ME_Y = Number(JSON_VALUE);
            //#--------------------------------------------------------------
            JSON_VALUE = JSON_PARSE.Z;
            if (typeof JSON_VALUE === "undefined") return 1;
			Read_ME_Z = JSON_VALUE;
            //#--------------------------------------------------------------
			Read_CODE_STEP ++;
        	break;		
        ...
        case 18://OUTPUT_UNIX_TIME
          ...
          //#--------------------------------------------------------------
        	...
        	break;				
        case 20://Read File Data
        	//TERMINAL(FROM_SER + evt.data + '\n');
        	//#-------------------------------------------------------------- 
            JSON_VALUE = JSON_PARSE.X;
        	if (typeof JSON_VALUE === "undefined") return 1;
            Read_FI_X = Number(JSON_VALUE);
            //#--------------------------------------------------------------
            JSON_VALUE = JSON_PARSE.Y;
        	if (typeof JSON_VALUE === "undefined") return 1;
            Read_FI_Y = Number(JSON_VALUE);
            //#--------------------------------------------------------------
            JSON_VALUE = JSON_PARSE.Z;
            if (typeof JSON_VALUE === "undefined") return 1;
			Read_FI_Z = JSON_VALUE;
            //#--------------------------------------------------------------
			Read_FILE_STEP ++;
        	break;
        ...
        case 24://ROOM
            ...
            JSON_VALUE = JSON_PARSE.D;
            if (typeof JSON_VALUE === "undefined") return 1;
            var D_Vel = JSON_VALUE;
            //#--------------------------------------------------------------
			DOM_Room_InPUT[R_Vel] .value(D_Vel);
            //API_iot_CREATE_Device_InPut(D_Vel, S_Vel, CREATOR_SQL, 1);
            //API_iot_CHANGE__INPUT_Description(D_Vel,S_Vel,V_Vel);
            //#--------------------------------------------------------------  
            break;			
