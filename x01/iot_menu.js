...
document.write('<script type="text/javascript" src="js/menu/DOM__InOut.js"></script>');
document.write('<script type="text/javascript" src="js/menu/DOM__FILE.js"></script>');

var LOG_TERM_Text = "";
function LOG_TERMINAL(message) {
    LOG_TERM_Text += message;
    LOG_TERM.value(LOG_TERM_Text);
}
function LOG_TERMINAL_CLR() {
    LOG_TERM_Text = "";
    LOG_TERM.value("");
}
...
function setup__menu() {
	...
	dom__InOUT = new OBJECT__DOM__InOUT();
	dom__File = new OBJECT__DOM__FILE();
	
	
	DOM_alert = createInput();
	///DOM_alert.style('width', '340px');
	DOM_alert.style('width', (W1.SizeW-6) +'px');
	DOM_alert.style('height', '140px');
	DOM_alert.style('textAlign', 'CENTER');
	DOM_alert.style('background-color', '#ffdd99');
	DOM_alert.value("Please Wait !!!");
	DOM_alert.style('fontSize','large'); 
	DOM_alert.style('fontSize','x-large');
	DOM_alert.hide();
	
	
	LOG_TERM = createElement('textarea', 'im an h2 p5.element!');
    LOG_TERM.style('height', '1320px');
	LOG_TERM.style('width', '520px');
    LOG_TERM.show();
	LOG_TERMINAL_CLR();
	LOG_TERMINAL("hiii" + "\n");
...
function dom__W2_right_left( value ) {
	dom__W2_right_left_status = value;
  ...
  // FILE #######################################
  dom__FI_P3.position(W2.X + 10, W2.Y + 18);
  dom__FI_P4.position(W2.X + 85, W2.Y + 18);
  dom__FI_P5.position(W2.X + 10, W2.Y + 68);
  dom__FI_Inp1.position(W2.X + 10, W2.Y + 50);
  dom__FI_Inp2.position(W2.X + 85, W2.Y + 50);
  dom__FI_Save_Descr.position(W2.X + 260, W2.Y + 49);
  
  
  dom__FI_progress1.position(W2.X + 105, W2.Y + 66 + 18);
  dom__FI_Sav_0.position(W2.X + 10, W2.Y + W2.SizeH - 50);
  dom__FI_Sav_1.position(W2.X + 10, W2.Y + W2.SizeH - 50);
  dom__FI_Sav_2.position(W2.X + 10, W2.Y + W2.SizeH - 50); 
  dom__FI_Rea_0.position(W2.X + W2.SizeW - 70, W2.Y + W2.SizeH - 50);
  dom__FI_Rea_1.position(W2.X + W2.SizeW - 70, W2.Y + W2.SizeH - 50);
  dom__FI_Rea_2.position(W2.X + W2.SizeW - 70, W2.Y + W2.SizeH - 50); 
  ...
  dom__DE_P4.position(W2.X + 84, W2.Y + 18);
  dom__DE_Inp1.position(W2.X + 10, W2.Y + 50);
  dom__DE_Inp2.position(W2.X + 84, W2.Y + 50);  
  //dom__DE_Save_Descr.position(W2.X + 260, W2.Y + 49);
  //dom__DE_Logo.position(W2.X+W2.SizeW/2-100, W1.Y+230);
  
  
  dom__DE_progress1.position(W2.X + 105, W2.Y + 66 + 18);
  dom__DE_P5.position(W2.X + 10, W2.Y + 68);
  dom__DE_Sav_0.position(W2.X + 10, W2.Y + W2.SizeH - 50);
  dom__DE_Sav_1.position(W2.X + 10, W2.Y + W2.SizeH - 50);
  dom__DE_Sav_2.position(W2.X + 10, W2.Y + W2.SizeH - 50);
  dom__DE_Rea_0.position(W2.X + W2.SizeW - 70, W2.Y + W2.SizeH - 50);
  dom__DE_Rea_1.position(W2.X + W2.SizeW - 70, W2.Y + W2.SizeH - 50);
  dom__DE_Rea_2.position(W2.X + W2.SizeW - 70, W2.Y + W2.SizeH - 50);   
  
   // InOUT #######################################
  dom__InOUT_P3.position(W2.X + 10, W2.Y + 18);
  dom__InOUT_P4.position(W2.X + 84, W2.Y + 18);
  dom__InOUT_P5.position(W2.X + 10, W2.Y + 18+45);
  dom__InOUT_P6.position(W2.X + 10, W2.Y + 50+45+30+15);
	
  dom__InOUT_Inp1.position(W2.X + 10, W2.Y + 50);
  dom__InOUT_Inp2.position(W2.X + 84, W2.Y + 50);
  dom__InOUT_Inp3.position(W2.X + 10, W2.Y + 50+45);   
  dom__InOUT_Inp4.position(W2.X + 10, W2.Y + 50+45+30);   
  dom__InOUT_Inp5.position(W2.X + 10, W2.Y + 50+45+30+30+15);  
  
  if(value !== 1){
	  dom__InOUT_Inp2.style('width', '165px');
	  dom__InOUT_Inp3.style('width', '240px');
	  dom__InOUT_Inp4.style('width', '240px');
	  dom__InOUT_Inp5.style('width', '240px');
  }else{
	  dom__InOUT_Inp2.style('width', '445px');
	  dom__InOUT_Inp3.style('width', '520px');
	  dom__InOUT_Inp4.style('width', '520px');
	  dom__InOUT_Inp5.style('width', '520px');  
  }
  ...
  dom__ME_Sav_0.position(W2.X + 10, W2.Y + W2.SizeH - 50);
  dom__ME_Sav_1.position(W2.X + 10, W2.Y + W2.SizeH - 50);
  dom__ME_Sav_2.position(W2.X + 10, W2.Y + W2.SizeH - 50);
  
  dom__ME_Rea_0.position(W2.X + W2.SizeW - 70, W2.Y + W2.SizeH - 50);
  dom__ME_Rea_1.position(W2.X + W2.SizeW - 70, W2.Y + W2.SizeH - 50);
  dom__ME_Rea_2.position(W2.X + W2.SizeW - 70, W2.Y + W2.SizeH - 50); 
  ...
  if(DOM_isSelected === "room")
  {
		  ...
		  for (var i = 0; i < 8; i++){
			DOM_Room_ON_Pic[i].position (W2.X + 15, W2.Y + N2 + N1*i);
			DOM_Room_OFF_Pic[i].position(W2.X + 15, W2.Y + N2 + N1*i);  
		  }
		  for (var i = 8; i < 16; i++){
			DOM_Room_ON_Pic[i].position (W2.X + 240, W2.Y + N2 + N1*(i-8));
			DOM_Room_OFF_Pic[i].position(W2.X + 240, W2.Y + N2 + N1*(i-8));  
		  }

		  for (var i = 0; i < 8; i++){
			DOM_Room_InPUT[i].position(W2.X + 15+50, W2.Y + N2 + 2 + N1*i);
		  }
		  
		  for (var i = 8; i < 16; i++){
			DOM_Room_InPUT[i].position(W2.X + 240+50, W2.Y + N2 + 2 + N1*(i-8)); 
		  } 
		  
		  if(dom__W2_right_left_status === 1){
			 for(var i = 8; i < 16; i++) DOM_Room_InPUT[i].show();
		  }else{
			 for(var i = 8; i < 16; i++) DOM_Room_InPUT[i].hide();
		  }	    
  }
  //######################################################################### 
			//var centerX = self.innerWidth;
            //var centerY = self.innerHeight;
  DOM_alert.position(W2.X+W2.SizeW+15 , W2.Y);
  
  LOG_TERM.position(W1.X+W1.SizeW+15 , W3.X);
  //DOM_alert.show();
